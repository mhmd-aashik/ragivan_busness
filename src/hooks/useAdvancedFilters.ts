...
export interface FilterOptions {
  ...
  // Sorting
  sortBy?: 'price-asc' | 'price-desc' | 'rating-desc' | 'newest' | 'popularity' | 'name-asc' | 'name-desc';
...
export function useAdvancedFilters() {
  ...
  const parseUrlParams = useCallback((): FilterOptions => {
    ...
    if (searchParams.get('availability')) {
      filters.availability = searchParams.get('availability') as 'in-stock' | 'out-of-stock' | 'all';
    }
    
    // Shipping filters
    if (searchParams.get('shipping')) {
      filters.shipping = searchParams.get('shipping') as 'free' | 'paid' | 'all';
    }
    ...
    if (searchParams.get('isBestSeller')) filters.isBestSeller = searchParams.get('isBestSeller') === 'true';
  ...
  const applyFilters = useCallback((products: Product[], filters: FilterOptions): Product[] => {
    ...
    if (filters.isBestSeller !== undefined) {
      filtered = filtered.filter(product => product.isBestSeller === filters.isBestSeller);
    }
  ...
  const fetchProducts = useCallback(async (filters: FilterOptions) => {
    try {
